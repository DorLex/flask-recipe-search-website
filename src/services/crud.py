...
def get_recipe_by_id(recipe_id: int) -> Recipes:
    query: Select = db.select(Recipes).filter_by(recipe_id=recipe_id)
...
def get_recipes() -> list[Recipes]:
    query: Select = db.select(Recipes).order_by(Recipes.recipe_id.desc()).limit(7)
...
def get_ingredients_ilike(ingredient: str) -> list[str]:
    query: Select = db.select(Ingredients.title).filter(
